...
public static class StringExtensions
{
    ...
    public static string NobleCardImagePath(this string path)
    {
        path = Path.Join(MainFile.ResPath, "images", "card_portraits", "noble", path);
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find noble card image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "card_portraits", "noble", "card.png");
    }
    ...
    public static string BigNobleCardImagePath(this string path)
    {
        ...
        return Path.Join(MainFile.ResPath, "images", "card_portraits", "big", "noble", "card.png");
    }
